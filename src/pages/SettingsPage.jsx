  ...
  return (
    <div className="settings-layout">
      ...
      <div className="settings-content page-stack">
        ...
        <section className="panel settings-section"><header><span><ScanLine size={20} /></span><div><h3>Leitura óptica</h3><p>Parâmetros de confiança para detectar preenchimentos.</p></div><Badge tone="green">Padrão recomendado</Badge></header><div className="range-setting"><div><strong>Rigor da marca confirmada</strong><p>Controla o contraste local necessário para confirmar uma resposta, compensando sombras na foto.</p></div><input type="range" min="25" max="60" value={threshold} onChange={(event) => updateOmrSetting('markThreshold', event.target.value)} /><b>{threshold}%</b></div><div className="range-setting"><div><strong>Zona de ambiguidade</strong><p>Controla quando uma marca fraca deve ser enviada para conferência manual.</p></div><input type="range" min="10" max="35" value={ambiguity} onChange={(event) => updateOmrSetting('ambiguityThreshold', event.target.value)} /><b>{ambiguity}%</b></div><div className="setting-note"><Info size={17} /> O leitor combina alinhamento da folha, contraste local e cor da tinta. Os valores padrão são recomendados para canetas azuis ou pretas.</div></section>
